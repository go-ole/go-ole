...
func TestClsidFromProgID_WindowsMediaNSSManager(t *testing.T) {
	...
	expected := &ole.GUID{0x92498132, 0x4D1A, 0x4297, [8]byte{0x9B, 0x78, 0x9E, 0x2E, 0x4B, 0xA9, 0x9C, 0x07}}

	coInitialize()
	defer CoUninitialize()
	actual, err := CLSIDFromProgID("WMPNSSCI.NSSManager")
	if err == nil {
		if !ole.IsEqualGUID(expected, actual) {
			t.Log(err)
...
func TestCreateInstance_WindowsMediaNSSManager(t *testing.T) {
	...
	if err == nil {
		...
		unknown, err := CreateInstance(actual, ole.IID_IUnknown)
		if err != nil {
			t.Log(err)
			t.Fail()
		}
